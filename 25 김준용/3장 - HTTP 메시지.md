...
시작줄과 헤더는 CRLF(ASCII 13, ASCII 10)로 분리되어있는 아스키 문자열이며, 본문은 선택적 데이터 덩어리다.
...
> 사유 구절: 상태 코드를 해석이 조금 쉽게 변향한 부분. 메시지 처리에 영향을 주지는 않는다.
...
이는 'Test Server Version 1.0'와 같은 의미이다.
